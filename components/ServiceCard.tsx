...
export default function ServiceCard({ service, featured = false }: ServiceCardProps) {
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-7 text-xl font-black tracking-tight text-white">{service.title}</h3>
      <p className="mt-7 max-w-sm text-lg leading-7 text-muted">{service.description}</p>
      <Link
        href="/contact"
